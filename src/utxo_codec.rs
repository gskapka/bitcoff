...
fn convert_utxos_json_string_to_utxos_jsons(
    utxo_json_string: &str
) -> Result<BtcUtxoAndValueJsons> {
    Ok(serde_json::from_str(utxo_json_string)?)
}

fn convert_utxo_json_to_utxo(
    utxo_and_value_json: &BtcUtxoAndValueJson
) -> Result<BtcUtxoAndValue> {
    Ok(
        BtcUtxoAndValue::new_serialized(
            utxo_and_value_json.utxo_value,
            hex::decode(&utxo_and_value_json.utxo_hex)?,
        )
    )
}

fn convert_utxo_jsons_to_utxos(
    utxo_jsons: BtcUtxoAndValueJsons
) -> Result<BtcUtxosAndValues> {
    utxo_jsons
        .iter()
        .map(convert_utxo_json_to_utxo)
        .collect::<Result<BtcUtxosAndValues>>()
}

pub fn get_utxos_from_utxo_json_string_and_add_to_state(
    state: State
) -> Result<State> {
...
mod tests {
    ...
    fn get_sample_utxo_json() -> BtcUtxoAndValueJson {
        convert_utxos_json_string_to_utxos_jsons(
            SAMPLE_UTXO_JSON_STRING
        )
            .unwrap()
            [0]
    ...
    fn should_test_utxo_json_serde_round_trip() {
        let utxo_json = get_sample_utxo_json(); 
        let utxo = convert_utxo_json_to_utxo(&utxo_json)
            .unwrap();
